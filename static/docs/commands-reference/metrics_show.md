...
- If type (via `-t`) and xpath (`-x`) are specified, they override the type and
  xpath defined (if any) in the corresponding DVC-file (usually, using
  `dvc metrics modify` command).
- If type (via `-t`) is specified and xpath (`-x`) is not, type will overwrite
  the type specified in DVC-file, no xpath will be considered, which will result
  in displaying all metrics from file.
- If type (via `-t`) is not specified and xpath (`-x`) is, DVC will try to read
  type from DVC-file and apply xpath provided in `-x`.

The type can be detected by the file extension automatically if the type is
supported.
