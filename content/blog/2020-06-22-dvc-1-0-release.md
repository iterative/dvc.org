...
3 years ago, I was concerned about good engineering standards in data science. I
wanted there to be Git for data. So I made DVC, which works as version control
for data projects. Technically, DVC codifies your data and machine learning
pipelines as text metafiles (with pointers to actual data in S3/GCP/Azure/SSH),
while you use Git for the actual versioning.

DevOps folks call this approach DataOps or MLOps but most of our users are ML
engineers and data scientists who do not use such terms often :)
...
tremendously improves human-readability of the `dvc.yaml`:
...
Another benefit of the run-cache is related to CI/CD systems for ML. The long
memory means users don't have to make auto-commits in their CI/CD system side -
see
...
Countless users have asked when we'd support metrics visualizations. Now it's
here: DVC 1.0 introduces metrics file visualization commands, `dvc plots diff`
and `dvc plots show`. This is a brand new functionality in DVC and it's _in
experimentation mode_ now.
...
We've done substantial work to optimize data management commands, like
...
production automation systems. Stability plays more and more important role.
...
![](/uploads/images/2020-05-04/dvc-benchmark.png)
...
continue working on DVC and bringing the best practices to the ML/AI space.
