...
description: |
  ...
  and outputs, and how to save models and data in continuous integration.
descriptionLong: |
  A roundup of technical Q&A's from the DVC community. This month, we discuss 
  customizing your DVC plots, the difference between external dependencies
  and outputs, and how to save models and data in continuous integration.
...
To fix this, convert your RSA key to the OpenSSH format. Tools like PuTTYgen and
MobaKeyGen can help you do this.
...
`model.pkl` file in your S3 bucket:

```dvc
$ dvc run -d s3://mybucket/model.pkl
```

DVC connects to the remote location (here, your S3 bucket) to check whether or
not the indicated file (`model.pkl`) has changed since the last time `dvc repro`
...
your GitHub Action/GitLab CI via CML. Then you'll use the following protocol in
your CI workflow (your workflow config file in GitHub/GitLab):
