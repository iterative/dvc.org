...
One thing to note here is when we are using `dvc checkout`, it does not affect
state of files and directories listed under `.dvcignore` as these are currently
untracked by DVC and `dvc checkout` synchronizes only tracked files and
directories with the versions specified in the current DVC-files. <br> See
