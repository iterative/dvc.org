...
4. Create a corresponding `.dvc` file to track the file, using its path (when
   used with `--to-remote`/`-o`, the external path becomes a local path) and
   hash to identify the cached data. The `.dvc` file lists the DVC-tracked file
   as an <abbr>output</abbr> (`outs` field). Unless the `--file` option is used,
