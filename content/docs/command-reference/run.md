...
usage: dvc run [-h] [-q | -v] [-d <path>] [-o <path>] [-O <path>]
               [-p <params>] [-m <path>] [-M <path>] [--file <filename>]
...
Unless the `--file` options is used, the stage file (DVC-file) is generated in
the current working directory and named `<file>.dvc`, where `<file>` is file
name of the first output (`-o`, `-O`, `-m`, or `-M` option). If neither `--file`
nor outputs are specified, the file name defaults to `Dvcfile`.
...
- `--file <filename>` - specify a path and/or file name for the stage file
  generated by this command (e.g. `--file stages/stage.dvc`). This overrides the
