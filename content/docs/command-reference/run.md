...
- `--no-exec` - write the stage to `dvc.yaml`, but do not execute the `command`.
  ...
  `dvc repro` to finish the operation.

  This is useful if you need to define a pipeline quickly before executing
  anything, and run all of its stages at once later (with `dvc repro`), or if
  the commands has been run manually before and/or it's outputs already exist,
  and you want to "DVCfy" the existing state of the project.
