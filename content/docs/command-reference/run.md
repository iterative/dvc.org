...
- In some situations we have executed a stage and later notice that some of the
  existing files/directories used by the stage as dependencies, or created as
  outputs, which are already in the workspace, are missing from `dvc.yaml`
  (`deps` and `outs` field respectively). We can
