...
- `-n <num>`, `--num <num>` - show experiments from the previous `num`
  commits (before the `--rev` baseline). For example `-n -2` includes
  experiments from `HEAD` and its 2 first ancestors.
