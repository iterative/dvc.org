...
Use `dvc exp run --run-all` to process the queue in the background. Adding `-j`
(`--jobs`), experiment queues can be run in parallel for better performance.
This creates a temporary <abbr>workspace</abbr> copy for each subprocess (in
`.dvc/tmp/exps`). Background experiment results are not reflected in the
workspace, like for regular experiments.
...
> makes no attempt to distribute stage commands among jobs. The order in which
> they were queues is also not guaranteed.

To run a single experiment in the background, use `dvc exp run --temp`. This way
if you can continue to work in your workspace while a long experiment runs.
...
- `--temp` - run this experiment in the background, and outside your workspace
  (in `.dvc/tmp/exps`). Useful to continue working while a long experiment runs.
...
- `--run-all` - run all queued experiments (see `--queue`) in the background,
  and outside your workspace (in `.dvc/tmp/exps`). Use `-j` to execute them
