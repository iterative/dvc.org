...
implicitly defined by the [stages](/doc/command-reference/run) (DVC-files with
...
results. `dvc repro` relies on the DAG definition that it reads from `dvc.yaml`
file.
...
command: by specifying stage `targets`( can be a `.dvc` file or one of the
stages in `dvc.yaml` file ), or by using the `--single-item`, `--cwd`, among
other options.
...
- `-s`, `--single-item` - reproduce only a single stage by turning off the
  ...
  (non-recursively) if multiple stages are given as `targets`.
