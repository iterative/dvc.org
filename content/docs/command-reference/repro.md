...
command: by specifying stage `targets`, or by using the `--single-item`,
`--cwd`, among other options.
...
- `-p`, `--pipeline` - reproduce the entire pipelines that the stage `targets`
