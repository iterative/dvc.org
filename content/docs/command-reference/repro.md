...
specifying specific reproduction `targets` (see [Arguments](#arguments) for
details), or by using certain command [options](#options), such as
`--single-item`.
...
stage name(s) (`./dvc.yaml` by default). DVC will reproduce the corresponding
parts of the project, as detailed below.
...
- For most **`.dvc` files**, `dvc repro` re-adds changed files or directories
