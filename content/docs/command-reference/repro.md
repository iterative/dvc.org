...
> See [`targets`](#targets-argument-optional) for more details.
...
specifying specific reproduction [`targets`](#targets-argument-optional), or by
using certain command [options](#options), such as `--single-item`.
...
### `targets` argument (optional)

> The default target is `./dvc.yaml` (if this argument is not provided).

Accepts one or more file or directory paths (to `.dvc` or `dvc.yaml` files), or
stage name(s) (`./dvc.yaml` by default). DVC will reproduce them as detailed
below.

- For **`dvc.yaml` files**, their [pipeline(s)](/doc/command-reference/dag) are
  checked for changes, and reproduced as needed (explained in the command
  [description](#description) above). E.g. `dvc repro pipes/linear/dvc.yaml`

- **Directory paths** can be provided if the `-R` option is included. `dvc.yaml`
  files are searched for (recursively) in the given dirs. E.g.
  `dvc repro -R subdir/`.
...
- Files and directories tracked by **`.dvc` files** given as `targets` are
  ...
  > Note that [frozen](/doc/command-reference/freeze) `.dvc` files are ignored.
