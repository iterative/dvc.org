...
- `sse_kms_key_id` - SSE-KMS key to use to encrypt data uploaded to S3, normally specified when the `sse` parameter is set to `aws:kms`. This parameter will be passed directly to AWS's S3 functions, so dvc supports any value that AWS supports, including both KMS key ids and aliases (e.g. `alias/my-key-name`).
