...
`dvc plot show` with `--html-template` option when Internet access is not
available

> ⚠️ This is a separate feature from
> [custom plot templates](/doc/command-reference/plots#custom-templates) to
> provide a custom HTML file for the _generated plots._

You can create an HTML file with `<script>` references to Vega libraries and put
`{plot_divs}` as a placeholder for the generated plots. You can supply this HTML
file with `--html-template` option.

### Example for the custom HTML template
