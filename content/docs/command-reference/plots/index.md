...
provide Vega JavaScript libraries. It's possible to supply a custom HTML file to
`dvc plot show` with `--html-template` option when the Internet access is not
available.

> ⚠️ This is a separate feature from
> [custom plot templates](/doc/command-reference/plots#custom-templates).
> `--html-template` allows to provide a custom HTML file to put _generated
> plots._

You can create an HTML file with `<script>` references to Vega libraries and put
`{plot_divs}` as a placeholder for the generated plots. See
[the example](/doc/command-reference/plots#example-for-the-html-template) for
details.
