...
users want to compare data from different data sources residing on the same
version of the project. For example, comparing training versus test results on
current branch.
...
- `y` - field name from which the Y axis data comes from. It can be a string,
  list or dictionary. If its a string or list, it is assumed that plot id will
...
- `title` - Plot title. Plot id by default.

Refer to the
[examples](/doc/command-reference/plots#example-simple-plot-definition) for more
syntax insight.
...
Upon running `dvc plots show/diff` DVC will collect plot outputs alongside the
definitions and display them conforming to their configuration. Note, that if
there are plot outputs in the project and they are also used in some plots
definitions, DVC will create separate rendering for the plot outputs and all
definitions using them.
...
writing plot definitions into the `dvc.yaml`.
...
#### Multiple data series plot definition
