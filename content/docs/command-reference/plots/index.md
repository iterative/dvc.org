...
It's possible to supply a custom HTML file to the `--html-template` option of
`dvc plot show` and `dvc plot diff` when you would like to customize the _output
template._
...
with a `{plot_divs}` marker. This way, you can customize the plots output.
[An example](/doc/command-reference/plots#example-for-the-html-template) that
uses this feature to provide plots that don't require Internet access is below.
...
default these employ
[online resources](https://vega.github.io/vega/usage/#embed) to present the
plots. There may be times when you need to produce plots without Internet
access, or want to customize the plots output to put some extra content, like
banners or extra text. DVC allows to replace the HTML file that contains the
final plots.
