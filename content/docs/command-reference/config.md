...
- `ui.theme` - Optionally indicates the color theme being used in the terminal.
  Accepts values `dark`, `light` and `None` (default). If not `None`, this
  option is used to alternate row's background color in the table generated by
  `dvc exp show`.
