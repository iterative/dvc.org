...
💡 Note that variables are supported with the special expression `${}`. See
[Variables in `dvc.yaml`](#variables-in-dvcyaml) to learn more about this
syntax.
...
### Variables in `dvc.yaml`

To avoid repetition, `dvc.yaml` supports variables with the following syntax.
Any YAML values are supported? but they are meant for simple data such as
numbers or strings.

Either a [parameters file](/doc/command-reference/params) or a special `vars`
section can contain the variable definitions. Both can coexist as well, the
former? taking precedence (if a repeated variable name is defined in `vars`, its
...
Alternatively, the `vars` can be included in the same `dvc.yaml` file:
...
```
${var}
# nested values can be accessed through `. (period)
${var.name}
# list uses "[]" syntax, which is just a sugar syntax for "."
${var.list[0]}
# escape parametrization
\${HOME}
```
