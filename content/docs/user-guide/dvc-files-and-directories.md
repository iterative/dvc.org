...
- `cmd` (always present): Executable commands defined in this stage. This could
  be either a single command or a list of commands that should be executed
  sequentially. The failure of one of these commandes will result in the failure
  of the stage. Note that each command is passed as is to the shell.
