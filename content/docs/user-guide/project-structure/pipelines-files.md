...
| `live`           | Define [live output](/doc/dvclive), and optionally its default configuration (avialable subfields are in [Dvclive section](/doc/dvclive/dvclive-with-dvc)) and whether or not this output is <abbr>cached</abbr> (`true` by default). See the `--live-no-cache` option of `dvc run`.      |
