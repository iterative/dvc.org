...
| `md5`  | (only for <abbr>imports</abbr>) MD5 hash of the import `.dvc` file itself.                                                                                                                                                    |
...
- Comments can be added to `.dvc` files using the `# comment` format.
...
> overwritten by `dvc add`, `dvc move`, `dvc import`, or `dvc import-url`.
